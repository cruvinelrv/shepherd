...
## 3. DDD & Project Management 🚧 _Alpha Development_
...
Ask questions about your project (or your whole multi-repo workspace) straight from the terminal, backed by Gemini.
...
# Configure the model and API key (or set the GEMINI_API_KEY env var instead)
shepherd ai config
```
...
```sh
# One-shot prompt
shepherd ai "what does this project do?"

# Pipe file content in
cat CHANGELOG.md | shepherd ai "summarize the latest changes"

# Interactive chat — run with no prompt in a real terminal
shepherd ai

# Include every project registered in .shepherd/workspace.yaml as context
shepherd ai --scope workspace "which services call shepherd_bff?"
```
Every prompt automatically includes local project context (`.shepherd/project.yaml`, `.shepherd/environments.yaml`, `devops/domains.yaml`). Add `--scope workspace` to also include a summary of every project in `.shepherd/workspace.yaml` — the multi-repo catalog generated by Shepherd Studio or `shepherd init` — instead of just the current one.
