...
Future<bool?> addDomainController(String domainName,
    {bool promptIfMissing = false}) async {
  final file = File('.shepherd/domains.yaml');
  ...
  final exists = domains.any((d) =>
      d is Map &&
      (d['name'] as String?)?.toLowerCase() == domainName.toLowerCase());
  if (exists) {
    return false;
  }
  if (promptIfMissing) {
  ...
  final updated = Map<String, dynamic>.from(
      yaml is YamlMap ? Map<String, dynamic>.from(yaml) : {});
