import 'package:shepherd/src/menu/presentation/cli/user_active_utils.dart';
...
import 'init_mode_prompt.dart';

Future<void> showInitMenu() async {
  // [TEMPORARILY HIDDEN IN v0.3.3]
  ...
  domains: []
''';
    await workspaceConfigFile.create(recursive: true);
    await workspaceConfigFile.writeAsString(workspaceContent);
    print('workspace.yaml created in .shepherd/.');
  }
  ...
  if (!shepherdDir.existsSync()) {
    ...
    print('Created .shepherd directory.');
  }

  if (!devopsDir.existsSync()) {
    await devopsDir.create(recursive: true);
    print('Created devops directory.');
  }
  ...
  try {
    // 0. Prompt Mode
    final initMode = await promptInitMode();
    ...
    while (true) {
      ...
      if (!environments.containsKey(env)) {
        ...
        if (branch != null && branch.isNotEmpty) {
          ...
          print('Environment "$env" → branch "$branch" added.');
        } else {
    ...
    // 2. Domain registration (Only in Full Mode)
    String? domainName;
    if (initMode == 'full') {
      domainName = await promptDomainName(allowCancel: true);
      ...
      // Após registrar owners, cria user_active.yaml escolhendo usuário
      await selectAndSetActiveUser(configDb);
    } else {
      // Automation Mode: Register a single user and set active
      print('\n--- Active User Registration ---');
      stdout.write('Enter your first name: ');
      final firstName = stdin.readLineSync()?.trim();
      if (firstName == null) throw ShepherdInitCancelled();
      ...
      final userId = await configDb.insertPerson(
        firstName: firstName,
        lastName: lastName,
        email: email,
        type: 'developer',
      ...
      final userMap = await configDb.getPersonById(userId);
      if (userMap != null) {
        await writeActiveUser(userMap);
        print('Active user set: $firstName $lastName');
      }
    }
    ...
    if (repoType == 'github') {
      if (initMode == 'full') {
        ...
        // In automation mode, just ask for the current user's github if not set
        final activeUser = await readActiveUser();
        if (activeUser != null &&
            (activeUser['github_username'] == null ||
                activeUser['github_username'].toString().isEmpty)) {
          ...
          if (ghUser != null && ghUser.isNotEmpty) {
            ...
            await writeActiveUser(updatedUser);
          }
        }
      }
    }
