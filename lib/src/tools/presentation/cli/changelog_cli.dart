...
class ChangelogCli {
  ...
  Future<String> promptChangelogType() async {
    stdout.write("Is this branch for update or change? (Type 'update' or 'change'): ");
    final input = stdin.readLineSync()?.trim().toLowerCase();
    if (input == null || (input != 'update' && input != 'change')) {
      throw Exception("Invalid changelog type. Please type 'update' or 'change'.");
    }
    return input;
  }
  ...
  Future<void> ensureChangelogFromReference({String referenceBranch = 'main'}) async {
    final result = Process.runSync('git', ['checkout', referenceBranch, '--', 'CHANGELOG.md']);
