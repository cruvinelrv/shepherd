...
Future<void> runFlowCommand(List<String> arguments) async {
  ...
  parser.addOption('base-branch', help: 'Base branch for PR (default: principal branch)');
  ...
  try {
    ...
    if (!noPr) {
      ...
      if (providedTitle != null && providedTitle.isNotEmpty) {
        ...
        prTitle = (inputTitle == null || inputTitle.isEmpty) ? 'Release v$newVersion' : inputTitle;
      } else {
      ...
      if (remoteUrlResult.exitCode == 0) {
        ...
        if (match != null && match.groupCount >= 1) {
          ...
          if (prBodyOption != null && prBodyOption.isNotEmpty) {
            ...
            final logResult = await Process.run('git', ['log', '--pretty=format:* %s', '${baseForPr}..${releaseBranch}']);
            if (logResult.exitCode == 0 && (logResult.stdout as String).trim().isNotEmpty) {
          ...
          if (token != null && token.isNotEmpty) {
            ...
            final payload = jsonEncode({
              ...
              'head': releaseBranch,
              'base': baseForPr,
              'body': prBody.isNotEmpty ? prBody : 'Release version v$newVersion generated by Shepherd CLI.',
            });
            ...
            if (response.statusCode == 201) {
              print('\x1B[32mPull Request successfully created via GitHub API!\x1B[0m');
            } else {
              print('\x1B[31mFailed to create Pull Request via API (status ${response.statusCode}). Falling back to gh CLI...\x1B[0m');
          ...
          if (token == null) {
            ...
            if (ghCheck.exitCode == 0) {
              ...
              if (authCheck.exitCode == 0) {
                ...
                print('PR URL: https://github.com/$repository/compare/$baseForPr...$releaseBranch');
              }
            } else {
...
Future<String> _generatePrBodyFromCommits(String base, String head) async {
  final result = await Process.run('git', ['log', '--pretty=format:* %s', '$base..$head']);
